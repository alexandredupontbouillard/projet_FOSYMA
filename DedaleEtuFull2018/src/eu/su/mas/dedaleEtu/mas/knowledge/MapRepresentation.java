...
public class MapRepresentation implements Serializable {
	...
	// permet de récupérer tous les noeuds de la carte
	public List<Case> getAllNodes() {
		...
		while (ite.hasNext()) {
			...
			if (x.getAttribute("ui.label") != null) {
				...
				b = s != null;
				result.add(new Case(x.getAttribute("ui.label"), x.getAttribute("tresor"), x.getAttribute("serrure"),
						x.getAttribute("force"), b, x.getAttribute("date")));
			}
		}
		return result;

	}// renvoit tous les trésor de la carte, sert pour les collecteurs
	...
	public List<String> getAlltreasure() {
		...
		result.sort(new Comparator<Couple<String, Integer>>() {
	...
	// renvoit tous les trésor fermés, sert pour les exploreurs
	public List<String> getAlltreasureClosed() {
		...
		while (ite.hasNext()) {
			...
			if (x.getAttribute("ui.label") != null && (int) x.getAttribute("tresor") != 0
					&& !(boolean) x.getAttribute("coffre_ouvert")) {
				String s = x.getAttribute("ui.class");
				b = s != null;
				result.add(new Couple<String, Integer>(x.getAttribute("ui.label"), x.getAttribute("serrure")));
			}
	...
	public List<Couple<String, String>> getAllEdges() {
		...
		while (ite.hasNext()) {
			e = ite.next();
			result.add(new Couple<String, String>(e.getSourceNode().getAttribute("ui.label"),
					e.getTargetNode().getAttribute("ui.label")));
		}
		return result;
	}
	...
	public Case getNode(String id) {
		...
		boolean b = s != null;
		return new Case(id, n.getAttribute("tresor"), n.getAttribute("serrure"), n.getAttribute("force"), b,
				n.getAttribute("date"));
	}
	...
	// ajoute un noeud a la carte avec ses informations si sa date est ultérieur à
	// celle déjà sauvegardé (si elle existe sinon l'ajoute simplement)
	...
	// récupère le chemin vers le noeud ouvert le plus proche
	...
	public List<String> syloPose() {
		...
		for (int i = 0; i < voisins.size(); i++) {
			...
			if (voisins.get(i).get(0).getDegree() > 1) {
				degree = ((compteur - voisins.get(i).get(0).getDegree())
						/ (voisins.get(i).get(0).getDegree() * (voisins.get(i).get(0).getDegree() - 1)));
			} else {
		...
		coeffClust.sort(new Comparator<Couple<Node, Float>>() {
			...
			public int compare(Couple<Node, Float> e1, Couple<Node, Float> e2) {
				if ((int) e1.getLeft().getAttribute("tresor") > 0) {
					...
					return -1;
				} else if (Integer.parseInt(e1.getLeft().getAttribute("ui.label")) > Integer
						.parseInt(e2.getLeft().getAttribute("ui.label"))) {
					return 1;
				} else {
					return -1;
				}
